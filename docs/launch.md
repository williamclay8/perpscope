...
Built PerpScope: a read-only Percolator risk cockpit + terminal adapter kit for Solana perps builders.

It turns decoded slab/account/oracle/execution logs into a cockpit with liquidation runway, protocol freshness, execution quality, and receipt history.
...
1. Built PerpScope: a read-only Percolator risk cockpit + terminal adapter kit for Solana perps builders.
...
3. v0.1 includes health, liquidation runway, protocol freshness, account risk, execution quality, flags, and a receipt timeline for spread, impact, 1m/5m markout, latency, priority fee, and source time.

4. It accepts PerpScope snapshots, captured CLI stdout, `list-markets`, `slab:*`, `best-price`, `execution:receipts`, and read-only RPC fixtures.

5. No wallet connection. No signing. No sending. No routing. Just observability and simulation.
...
- X: use the short post or thread above with `docs/screenshots/perpscope-receipts.png`.
